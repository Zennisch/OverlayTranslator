...
class ModelWrapper(ABC):
    ...
    async def download(self, force=False):
        ...
        if force or not self.is_downloaded():
            # Run without interactive prompts (prompt_yes_no removed)
            try:
                await self._download()
                self._downloaded = True
            except ModelVerificationException as exc:
                print("Failed to verify model signature. Non-interactive session aborting.")
                raise exc
